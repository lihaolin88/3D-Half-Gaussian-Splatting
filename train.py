...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,normal_lr, finetune):
    first_iter = 0
    tb_writer = prepare_output_and_logger(dataset)
    gaussians = GaussianModel(dataset.sh_degree)
    # finetune = True
    scene = Scene(dataset, gaussians)  #, finetune=finetune)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if iteration % 5000==0:
                if finetune:
                    gaussians.optimizer.param_groups[1]['lr'] /= 1.4
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[1_000, 2_500, 5_000, 7_000, 15_000, 30_000]) #7_000, 18_000, 22_000, 28_000, #[7_000, 30_000])
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.normal_lr, args.finetune)
